...
public class ProceduralAnimBase : MonoBehaviour
{
   // [Serializable]
    ...
    void Update()
    {
       ...
       // FloorRay = new Ray(StartPoint.position, Vector3.down);
        ...
        else
        {
            ...
            Debug.DrawRay(FloorRay.origin, FloorRay.direction);
            if (Physics.Raycast(FloorRay, out hit, BotBody.DesiredHeight, GroundLayer))
            {
                GroundHitLocation = hit.point;
            }
        }
        ...
        NextStepPos2 = NextStepPos + BotBody.transform.forward * Settings.MaxStrideLength * 1f;
        ...
        StepSpeed = BotBody.moveSpeed * 3;

        if (!moving && MovementAllowed && Vector3.Distance(EndPoint.position, NextStepPos) > Settings.MaxStrideLength)
        {
            moving = true;
