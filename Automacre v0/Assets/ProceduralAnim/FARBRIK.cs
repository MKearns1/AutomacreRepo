    ...
    void solver()
    {
        ...
        else
        {
            ...
            for (int i = 0; i < iterations; i++)
            {
                ...
                /*if (GoingTowardsBase)
                {
                    bool KeepGoingDown = cur.parent != transform;

                    while (KeepGoingDown)
                    {
                        Vector3 Pos = GetPositionAlong(cur.position, );

                        if (cur == Joints[Joints.Count - 1].Joint)           // < THIS IS THE END OR LEAF JOINT
                        {
                            Pos = TargetPoint;
                        }
                        else
                        ...
                        {
                            KeepGoingDown = false;
                            GoingTowardsBase = false;
                        }
                    }
                }
                else
                {
                    bool KeepGoingUp = cur.childCount > 0;
